  ...
  return useQuery<MatchDecision[]>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        // User might not have any matches yet
        return [];
      }
    },
    enabled: !!actor && !actorFetching && !!userId,
...
export function useSendMessage() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      return actor.sendMessage(recipient, content, timestamp);
    },
...
export function useGetFeed(matchedUsers: Principal[]) {
  ...
  return useQuery<PhotoPost[]>({
    ...
    queryFn: async () => {
      ...
      return actor.getFeed(matchedUsers, BigInt(100));
    },
